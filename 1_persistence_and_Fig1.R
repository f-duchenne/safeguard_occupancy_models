...
############# WAFFLE PART, should we still do that ? If yes we should start again from the database to be sure numbers are up to date:

#number of records per species per period
dat=datf %>% group_by(taxo_group,time_period,TAXON) %>% summarise(prop_sp=length(unique(TAXON))/unique(ntot_spec_europe))

#read the data-----
iucn <- fread(paste0(project_folder,"data/Pollinators_IUCN.csv"))
head(iucn)
iucn$Species <- gsub("_new", "", iucn$Species)
iucn$Species[which(!iucn$Species %in% dat$species)]
dat$TAXON[!(dat$TAXON %in% iucn$Species)]
#IGNORE FOR NOW
dat <- merge(dat, iucn, by.x = "TAXON", by.y = "Species")
dim(dat)
head(dat)

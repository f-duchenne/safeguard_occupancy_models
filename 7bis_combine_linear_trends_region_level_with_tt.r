...
for(jj in unique(bidon$taxo_group)){
	for(j in 1:length(baselines_vec)){
		...
		save(lis_bas,file=paste0(project_folder,"data/final_and_intermediate_outputs/models/model_with_tt_with_GBIF_",baselines_vec[j],"_",jj,".RData"))
	}
}
...
for(jj in unique(bidon$taxo_group)){
	for(j in 1:length(baselines_vec)){
		...
		lis_bas=list(model,model2)
		save(lis_bas,file=paste0(project_folder,"data/final_and_intermediate_outputs/models/model_total_with_tt_with_GBIF_",baselines_vec[j],"_",jj,".RData"))
	}
}

...
modeltt=glmmTMB(Y~log.list.length.c_s+log.list.count_s+(1|region_50/MONTH_2),family=binomial,data=dat,ziformula=~ar1(period.num2 + 0 | region_50)+(1|site),control=glmmTMBControl(optCtrl =list(iter.max=1e5,eval.max=1e3)))
}else{
modeltt=glmmTMB(Y~log.list.length.c_s+log.list.count_s+(1|MONTH_2),family=binomial,data=dat,ziformula=~ar1(period.num2 + 0 | group)+(1|site),control=glmmTMBControl(optCtrl =list(iter.max=1e5,eval.max=1e3)))
}

### If it did not converge try to change the solver
b=0
while((modeltt$fit$convergence!=0 | is.na(AIC(modeltt))) & b<3){
b=b+1
if(length(unique(dat$region_50))>1){
modeltt=glmmTMB(Y~log.list.length.c_s+log.list.count_s+(1|region_50/MONTH_2),family=binomial,data=dat,ziformula=~ar1(period.num2 + 0 | region_50)+(1|site),control=glmmTMBControl(optimizer=optim,optArgs=list(method=optim_vec[b])))
}else{
modeltt=glmmTMB(Y~log.list.length.c_s+log.list.count_s+(1|MONTH_2),family=binomial,data=dat,ziformula=~ar1(period.num2 + 0 | group)+(1|site),control=glmmTMBControl(optimizer=optim,optArgs=list(method=optim_vec[b])))
}
}
aic_vec[jj]=AIC(modeltt)
if(jj==1){modelt=modeltt}
if(length(aic_vec[!is.na(aic_vec)])>0 & !is.na(aic_vec[jj])){
if(aic_vec[jj]==min(aic_vec[!is.na(aic_vec)])){modelt=modeltt}
}
}
...
lili2=list(modelt,aic_vec,period_tab,nsurvey_tot,nsurvey_used,scaling,regions)

save(lili2,file=paste0("results/model_non_linear",tab$species[i],".RData"))
#}
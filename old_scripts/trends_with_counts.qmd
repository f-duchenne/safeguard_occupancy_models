...
trendsf=trendsf %>% group_by(region_50) %>% mutate(pred_quant95=quantile(trend,probs=0.95))

model=lmer(trend~region_50+(1|species),data=subset(trendsf,trend<pred_quant95))
model=rma(trend ~ 0+region_50, sei=sde,data=subset(trendsf,trend<pred_quant95), digits=5)
...
bb=nonl %>% group_by(group,years) %>% summarise(med=exp(mean(log(pred))))
...
model=rma(x.trend ~ region_50+(1|species), sei=SE, method="FE", data=trendsf, digits=5)
model=lmer(x.trend~region_50+(1|species),data=trendsf)
b=ggpredict(model,"region_50")
